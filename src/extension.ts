...
async function create_name_input() {
	var option: vscode.InputBoxOptions = {
		...
		prompt: "Type your class name in PascalCase"
	};
...
export async function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('extension.createClass', async (args) => {
		...
		var classNameInput = await create_name_input();

		if (!can_continue(classNameInput)) return; // check for class name

		// TODO: A better dispatch mechanism would make it easier to add additional conventions
		var className = classNameInput as string;
		if (vscode.workspace.getConfiguration().get("cpp.creator.class.namingScheme") === 'kebab-case') {
			className = format_kebab_case(className);
		}
		let fileName = classNameInput as String
		if (vscode.workspace.getConfiguration().get("cpp.creator.file.namingScheme") === 'kebab-case') {
